#!/usr/bin/env python3
# compacity-fonts
#
# Copyright 2023 Aleksandar Radivojevic
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# config.py: dynamic configuration file for the font

import psMat

from . import Options, generator as g, PROJECT_ROOT
from builder import font

# TODO regenerate the project file, it retains old defined glyphs so the whole
# code is not parametric anymore and now there is 'BLOCK.{index}.0' defined
# cause of a bug...

# TODO: remove the translate and use gylph.translate instead
# returns names of glyphs not file paths
def block(x: str, translate=None):
    '''Transforms a string into list of base glyph names'''
    blocks = []

    top = x[:3]
    middle = x[4:6]
    bottom = x[7:]

    for i in range(3):
        if top[i] != ' ':
            blocks.append(1 + i)

    if middle == '||':
        blocks.append(0)
    else:
        for i in range(2):
            if middle[i] != ' ':
                blocks.append(4 + i)

    for i in range(3):
        if bottom[i] != ' ':
            blocks.append(6 + i)

    blocks = [ g.BBLOCK + str(x) for x in blocks ]

    if translate is not None:
        return [ (x, psMat.translate(translate)) for x in blocks ]
    else:
        return blocks

def defaults(glyph: font.GlyphBuilder):
    '''Sets defaults for each glyph, run with GlyphBuilder.do(..)'''
    glyph.set_unlink_overlap_on_save() \
        .comment('AUTOGENERATED DO NOT EDIT')

# TODO: add a way to add the blocks to an existing font
def gen(font: font.Font, options):
    '''Modified the font'''
    # REDO THE SCALING TO OPTIONS THIS IS GETTING TOO MESSY
    CHR_WIDTH = g.BLOCK_WIDTH
    SPC_WIDTH = g.BLOCK_WIDTH

    # separated variant make the characters not touch eachother, this should
    # not be used for all characters as special characters are already way
    # bigger than single block width (128)
    if Options.SPARSE in options:
        CHR_WIDTH = CHR_WIDTH * 2

    if Options.SENTENCE_LINE in options:
        SPC_WIDTH = g.BLOCK_WIDTH * 2

    for i in range(g.BLOCK_COUNT + 1):
        font.glyph().name(g.BBLOCK + str(i)) \
                    .clear() \
                    .do(defaults) \
                    .outline(g.OUTPUT_DIR / g.NAME_FORMAT.format(index=i)) \
                    .width(0) \
                    .color(0xf22929)

    # remove the line outline to make it invisible in variants
    if Options.NOLINE in options:
        font.glyph().name(g.BBLOCK + '0') \
                    .clear() \
                    .do(defaults) \
                    .width(0) \
                    .color(0xf22929)

    ## LETTERS ##
    LETTERS = {}

    LETTERS['a'] = '  # ||    '
    LETTERS['b'] = '  # || #  '
    LETTERS['c'] = '    || ## '
    LETTERS['d'] = ' ## ||    '
    LETTERS['e'] = ' #  ||    '
    LETTERS['f'] = '### ||    '
    LETTERS['g'] = '    || #  '
    LETTERS['h'] = ' ## || #  '
    LETTERS['i'] = '# # ||    '
    LETTERS['j'] = '# # || ## '
    LETTERS['k'] = ' #  ||  # '
    LETTERS['l'] = ' ## || ## '
    LETTERS['m'] = '  # || ###'
    LETTERS['n'] = '  # || # #'
    LETTERS['o'] = '#   ||    '
    LETTERS['p'] = '##  ||    '
    LETTERS['q'] = ' ## || ###'
    LETTERS['r'] = '    ||  # '
    LETTERS['s'] = '    ||  ##'
    LETTERS['t'] = '    || ###'
    LETTERS['u'] = '##  ||  ##'
    LETTERS['v'] = '##  || ## '
    LETTERS['w'] = '##  || ###'
    LETTERS['x'] = '### || ## '
    LETTERS['y'] = ' ## || ###'
    LETTERS['z'] = '# # || ###'

    # NOTE: add extra letters here

    # uppercase letters are gonna be the same
    for key, value in dict(LETTERS).items():
        LETTERS[key.upper()] = value

    for k, v in LETTERS.items():
        font.glyph().char(k) \
                    .clear() \
                    .do(defaults) \
                    .width(CHR_WIDTH) \
                    .refs(block(v)) \
                    .color(0xd97dfa)

    ## NUMBERS ##
    NUMBERS = {}

    # i used to generate these from binary but it's messy and well it ain't
    # gonna change so why not just predefine it
    NUMBERS['0'] = '#        #'
    NUMBERS['1'] = '#       ##'
    NUMBERS['2'] = '#       # '
    NUMBERS['3'] = '#       ##'
    NUMBERS['4'] = '#      #  '
    NUMBERS['5'] = '#      # #'
    NUMBERS['6'] = '#      ## '
    NUMBERS['7'] = '#      ###'
    NUMBERS['8'] = '#     #   '
    NUMBERS['9'] = '#     #  #'

    for k, v in NUMBERS.items():
        font.glyph().char(k) \
                    .clear() \
                    .do(defaults) \
                    .width(CHR_WIDTH) \
                    .refs(block(v)) \
                    .color(0x87dbfa)

    # these characters wont be rendered at all
    # TODO maybe add one small character that can indicate that there are
    # useless characters there
    ZERO_WIDTH = r';:-`—@#$%^&*~/\|_=+~`<>'

    for k in ZERO_WIDTH:
        font.glyph().char(k) \
                    .clear() \
                    .do(defaults) \
                    .width(0) \
                    .color(0x3b0f1e)

    ## SYMBOLS ##
    symbol_color = 0xdede59

    # this is the glyph that replaces space in SENTENCE_LINE variants
    font.glyph().name('blockspace') \
                .clear() \
                .do(defaults) \
                .refs(block('    ||    ')) \
                .transform(psMat.scale(2, 1)) \
                .width(SPC_WIDTH) \
                .color(symbol_color)

    font.glyph().char(' ') \
                .clear() \
                .do(defaults) \
                .width(SPC_WIDTH * 2) \
                .color(symbol_color)

    font.glyph().char('.') \
                .clear() \
                .do(defaults) \
                .refs(block('         #', (g.BLOCK_WIDTH, 0))) \
                .width(g.BLOCK_WIDTH * 5) \
                .color(symbol_color)

    font.glyph().char(',') \
                .clear() \
                .do(defaults) \
                .refs(block('        ##', (g.BLOCK_WIDTH, 0))) \
                .width(g.BLOCK_WIDTH * 3) \
                .color(symbol_color)

    font.glyph().char('!') \
                .clear() \
                .do(defaults) \
                .refs(block('### ## # #', (g.BLOCK_WIDTH, 0))) \
                .width(g.BLOCK_WIDTH * 3) \
                .color(symbol_color)

    font.glyph().char('?') \
                .clear() \
                .do(defaults) \
                .refs(block('# # ## ###', (g.BLOCK_WIDTH, 0))) \
                .width(g.BLOCK_WIDTH * 3) \
                .color(symbol_color)

    for i in '“”"‘’\'':
        font.glyph().char(i) \
                    .clear() \
                    .do(defaults) \
                    .refs(block('##         ', (g.BLOCK_WIDTH, 0))) \
                    .width(g.BLOCK_WIDTH * 3) \
                    .color(symbol_color)

    for i in '({[':
        font.glyph().char(i) \
                    .clear() \
                    .do(defaults) \
                    .refs(block('#         ', (g.BLOCK_WIDTH * 2, 0))
                        + block('### ## ###', (g.BLOCK_WIDTH, 0))) \
                    .width(g.BLOCK_WIDTH * 4) \
                    .color(symbol_color)

    for i in ')]}':
        font.glyph().char(i) \
                    .clear() \
                    .do(defaults) \
                    .refs(block('### ## ###', (g.BLOCK_WIDTH * 2, 0))
                        + block('         #', (g.BLOCK_WIDTH, 0))) \
                    .width(g.BLOCK_WIDTH * 4) \
                    .color(symbol_color)

    if Options.SENTENCE_LINE in options:
        # TODO this should not be hardcoded
        font.merge_feature(PROJECT_ROOT / 'features' / 'sentence-line.fea')
